...
| `.config/zsh/rc.zsh` | zsh 起動時に直接 `source`（symlink ではない） | [zsh.nix](nix/home/zsh.nix) |
...
**GUI 付き Linux** で macOS 風のキー再マップ（xremap）も入れたい場合は、構成名を
`$USER-gui@…` にする（初回のみ xremap 用の root 設定が必要）。詳細は
[docs/nix-vm.md](docs/nix-vm.md) の「GUI 付き Linux」節を参照。

```sh
nix run home-manager/master -- switch -b backup --flake ~/dotfiles#$USER-gui@$(uname -m)-linux
```
