...
| リサイズ（境界を左右へ） | `Cmd+Ctrl+←` / `Cmd+Ctrl+→` | `Ctrl+←` / `Ctrl+→` |
| リサイズ（境界を上下へ） | `Cmd+Ctrl+↑` / `Cmd+Ctrl+↓` | `Ctrl+↑` / `Ctrl+↓` |

> VSCode ではターミナル/エクスプローラー側からも `Ctrl+h/j/k/l` で戻れる（`keybindings.json` で定義）。
> ただしターミナル内の `Ctrl+l` だけはシェルの「画面クリア」を優先。
> リサイズは macOS の Mission Control（Ctrl+矢印）と衝突するため、VSCode では `Cmd+Ctrl+矢印` に割り当てている。
