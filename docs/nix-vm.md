...
| ターミナル / VSCode 設定 | `nix/home/{terminals,vscode}.nix` が symlink | (VM では対象外) |
| nvim / vim 設定 | `.config/nvim` / `.vimrc` を symlink | 同じ実体を symlink |

> **`install.sh` は Nix 非対応マシン用のフォールバック。** Nix で運用しているホスト
> (macOS / Linux とも) では実行しないこと。`zsh/zshenv` のリンク先 `~/.zshenv` は
> Home Manager も生成するため衝突し、activation が失敗する。また `zsh/zshrc` は
> Homebrew / pyenv / nvm 前提で Linux では動かない。shell 設定は `nix/home/zsh.nix` が担当する。
